...
int *pico_flash_read(long int FLASH_TARGET_OFFSET, size_t len) {
  const uint32_t *flash_target_contents =
      (const uint32_t *)(XIP_BASE + FLASH_TARGET_OFFSET);
...
int pico_flash_write(long int FLASH_TARGET_OFFSET, uint32_t flash_data[],
                     size_t num) {
  ...
  if (strcmp(stored_value, flash_data) ==
      0)  //++ Compares the stored and written data
  {
    printf(GREEN "\n COMMIT SUCCESSFULL \n" RESET);
    return "OK";
  } else {
...
void pico_flash_erase(long int FLASH_TARGET_OFFSET) {
  ...
  printf("[pico_flash_erase] erasing\n");
