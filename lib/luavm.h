...
int luaRunOnPitchBend(int index, int channel, int pitch,
                      float *volts, bool *volts_new, bool *trigger, float *gate) {
  // Check for global panic flag
  if (lua_global_panic_flag) {
    return LUA_ERRRUN; // Return error to indicate panic state
  ...
  lua_getfield(L, -1, "on_pitch_bend");  // Push envs[index].on_pitch_bend onto the stack
  if (!lua_isfunction(L, -1)) {          // Check if on_pitch_bend is a function
    lua_pop(L, 3);                       // Pop envs, envs[index], and on_pitch_bend
    return -1;
  }
  ...
  int result = lua_pcall(L, 2, LUA_MULTRET, 0);
  if (result != LUA_OK) {  // Call on_pitch_bend with 2 arguments
    printf("[luaRunOnPitchBend] error: %s\n", lua_tostring(L, -1));
    lua_pop(L, 3);  // Pop error message, envs[index], and envs (pcall consumed function+args)
    return result;  // Return error code
  }
